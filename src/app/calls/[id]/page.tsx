...
import { getAppointmentsInWindow, getDoctor, getHospital, getUnavailability } from "@/hms/mockHms";
import { callScript } from "@/lib/callScript";
import { callSummary } from "@/lib/status";
import { formatTime } from "@/lib/time";
import AnswerButtons from "./AnswerButtons";

export default async function CallSimulator({ params }: PageProps<"/calls/[id]">) {
  ...
  const appointments = await getAppointmentsInWindow(id);
      ...
      {current ? (
        <>
          <p className="mb-3 text-sm font-medium text-slate-500">
            Patient {appointments.length - toCall.length + 1} of {appointments.length}
          </p>
          <div className="grid gap-6 md:grid-cols-[1fr_320px]">
            ...
            <div className="rounded-[2rem] bg-slate-900 p-3 shadow-xl">
              <div className="rounded-[1.5rem] bg-slate-800 p-6 text-white">
                ...
                <p className="mt-2 text-2xl font-semibold">{current.patient.name}</p>
                <p className="tabular-nums text-slate-300">{current.patient.phone}</p>
                <div className="mt-3 flex flex-wrap gap-2 text-xs">
                  <span className="rounded-full bg-white/10 px-2 py-1">
                    Language: {current.patient.preferredLanguage}
                  </span>
                  <span className="rounded-full bg-white/10 px-2 py-1">
                    Appointment: {formatTime(current.startTime)}
                  </span>
                </div>
                ...
                <p
                  lang={
                    { English: "en", Tamil: "ta", Hindi: "hi" }[current.patient.preferredLanguage]
                  }
